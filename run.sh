...
BACKUP_CMD="mongodump --out /mongo-backup/"'${BACKUP_NAME}'" --host ${MONGODB_HOST} --port ${MONGODB_PORT} ${USER_STR}${PASS_STR}${DB_STR} ${EXTRA_OPTS}"
...
if ${BACKUP_CMD} ;then
    ...
    echo "   Backup failed"
    rm -rf /mongo-backup/\${BACKUP_NAME}
fi

if [ -n "\${MAX_BACKUPS}" ]; then
    while [ \$(ls /mongo-backup -N1 | wc -l) -gt \${MAX_BACKUPS} ];
    do
        BACKUP_TO_BE_DELETED=\$(ls /mongo-backup -N1 | sort | head -n 1)
        echo "   Deleting backup \${BACKUP_TO_BE_DELETED}"
        rm -rf /mongo-backup/\${BACKUP_TO_BE_DELETED}
    done
fi
echo "=> Backup done"
EOF
...
echo "${CRON_TIME} /backerup.sh >> /mongo_backup.log 2>&1" > /crontab.conf
